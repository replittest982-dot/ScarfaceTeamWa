...
async def cb_back(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer(f"👋 Главное меню\n{SEP}", reply_markup=main_kb(c.from_user.id))
    await c.answer()
...
async def cb_account_decision(c: CallbackQuery, bot: Bot):
    ...
    try: 
        await bot.send_message(target_uid, user_text, parse_mode="HTML")
    except: 
        pass
    await c.answer()
...
async def cb_bind_save(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer(guide, parse_mode="HTML")
    await state.clear()
    await c.answer()
...
async def cmd_num(m: Message, bot: Bot):
    ...
    try: 
        await bot.send_message(row['user_id'], f"⚡ <b>Ваш номер взяли!</b>\n📱 {mask_phone(row['phone'], 0)}\nОжидайте код.", parse_mode="HTML")
    except: 
        pass

# ==========================================
# 📩 ОБРАБОТКА SMS И CODE
# ==========================================
...
async def bridge_handler(m: Message, bot: Bot, state: FSMContext):
    # Игнорируем команды
    ...
    if current_state is not None:
        # Пользователь в каком-то состоянии FSM - не обрабатываем как мост
        return
...
async def cb_worker_actions(c: CallbackQuery, bot: Bot):
    ...
    async with get_db() as db:
        ...
        if row['worker_id'] != c.from_user.id: 
            await c.answer("🔒 Не ваш номер!", show_alert=True)
            return
        ...
        if act == "act":
            ...
            user_msg = "✅ Номер встал и работает!"
    ...
    try:
        if new_kb: 
            ...
            await c.message.edit_text(adm_msg)
    except TelegramBadRequest:
        pass
...
async def cb_sel_tariff(c: CallbackQuery, state: FSMContext):
    await state.clear()
    async with get_db() as db:
        tariffs = await (await db.execute("SELECT * FROM tariffs")).fetchall()
    ...
    try:
        ...
        await c.message.answer(f"📂 Выберите тариф для сдачи:\n{SEP}", reply_markup=kb.as_markup())
    await c.answer()
...
async def cb_pick_tariff(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer(msg, reply_markup=kb.as_markup(), parse_mode="HTML")
    await c.answer()
...
async def cb_my_queue(c: CallbackQuery):
    ...
    async with get_db() as db:
        ...
        if not my_nums:
            await c.answer("📭 Очередь пуста", show_alert=True)
            return
...
async def cb_help_menu(c: CallbackQuery, state: FSMContext):
    await state.clear()
    text = (
    ...
    try:
        ...
        await c.message.answer(text, reply_markup=help_kb(), parse_mode="HTML")
    await c.answer()
...
async def cb_ask_support(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer("✍️ Напишите ваш вопрос ниже:", reply_markup=kb.as_markup())
    await c.answer()
...
async def cb_adm_cast(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer("📢 Пришлите пост для рассылки (Текст/Фото/Видео):", reply_markup=kb.as_markup())
    await c.answer()
...
async def cb_adm_reports(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer("📊 Выберите период:", reply_markup=kb.as_markup())
    await c.answer()
...
async def cb_adm_tariffs(c: CallbackQuery, state: FSMContext):
    await state.clear()
    async with get_db() as db:
    ...
    try:
        ...
        await c.message.answer("🛠 Выберите тариф для изменения:", reply_markup=kb.as_markup())
    await c.answer()
...
async def cb_edit_t_price(c: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await c.message.answer(f"1️⃣ Введите новую **ЦЕНУ** для {target}:", parse_mode="Markdown")
    await c.answer()
            ...
            async with get_db() as db:
                ...
                for w in waiters:
                    ...
                    if (now_dt - st).total_seconds() / 60 >= CODE_WAIT_MINUTES:
                        ...
                        try:
                            await bot.send_message(w['user_id'], f"⏰ Время истекло. Номер {w['phone']} отменен.")
                        except:
                            pass
