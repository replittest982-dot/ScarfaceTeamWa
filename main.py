...
DB_NAME = "fast_team_v22.db" # База остается от v22, структура подходит
MSK_OFFSET = 3 
...
async def tariffs_kb_user():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT name, price, hold_info FROM tariffs") as c: rows = await c.fetchall()
    kb = []
...
# Worker Keyboards (SIMPLE & SECURE)
...
async def show_profile(c: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT reg_date FROM users WHERE user_id=?", (user_id,)) as cur: reg_date = (await cur.fetchone())[0].split(' ')[0]
...
async def set_topic(c: CallbackQuery):
    ...
    guide_text = (f"✅ Топик привязан к: **{t_name}**\n\n"
                  ...
                  f"1️⃣ Чтобы взять номер, пиши: **/take**\n"
                  f"2️⃣ Чтобы отправить код: **/sms номер код**\n"
                  f"3️⃣ **ВАЖНО:** Кнопки управления номером работают ТОЛЬКО у того, кто его взял!")
    await c.message.edit_text(guide_text, parse_mode="Markdown")

@router.message(Command("take") | Command("vzyat"))
async def cmd_take(message: types.Message, bot: Bot):
    # Механика взятия БЕЗ КНОПКИ
    chat_id = message.chat.id
    thread_id = message.message_thread_id if message.is_topic_message else 0
    key = f"topic_cfg_{chat_id}_{thread_id}"
    
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT value FROM config WHERE key=?", (key,)) as cur: t_res = await cur.fetchone()
        t_name = t_res[0] if t_res else None
        if not t_name: return # Молчим, если топик не настроен
        ...
        # Atomic update
        await db.execute("UPDATE numbers SET status = 'work', worker_id = ?, start_time = ? WHERE id = ? AND status = 'queue'", (message.from_user.id, datetime.utcnow().isoformat(), num_id))
        if db.rowcount == 0: await message.reply("⚠️ Кто-то успел взять раньше!"); return
        await db.commit()
    ...
    # Отправляем сообщение с защищенной клавиатурой
...
async def worker_fin_secure(c: CallbackQuery, bot: Bot):
    ...
    # Если нажал не владелец и не админ
    if owner_id != c.from_user.id and c.from_user.id != ADMIN_ID:
        await c.answer("🚫 НЕ ТРОГАЙ! Это чужой номер.", show_alert=True)
        return
...
async def sms_h(m: types.Message, cmd: CommandObject, bot: Bot):
    ...
    try:
        ...
        # Проверка, что отправляет тот, кто взял (или админ)
        if r and (r[1] == m.from_user.id or m.from_user.id == ADMIN_ID):
            ...
            await m.reply("🚫 Это не твой номер или он не в работе.")
    except: pass
...
async def main():
    ...
    print("🚀 v23.0 SECURE & CLEAN STARTED")
