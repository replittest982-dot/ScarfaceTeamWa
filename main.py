...
async def fsm_rep_gen(m: Message, state: FSMContext):
    ...
    await m.answer_document(BufferedInputFile(out.getvalue().encode(), filename=f"report_{hours}h.csv"), caption=f"📊 Отчет {hours}ч")
    await state.clear()
